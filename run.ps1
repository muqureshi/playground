...
Function setupMachine () {
    If (Get-Command docker-machine -errorAction SilentlyContinue) {
        If ((docker-machine ls | select-string -pattern "$SERVICE") -eq $null) {
            docker-machine create -d virtualbox --virtualbox-cpu-count "2" --virtualbox-memory "3072" --virualbox-disk-size "20000" $SERVICE
        }
    } Else {
...
Function setupDNS () {
    ...
    If($ip){
        ...
        If((gc $HostLocation) -contains $NewHostEntry){
            echo "$ip $dns already exists in host file"
        } Else {
